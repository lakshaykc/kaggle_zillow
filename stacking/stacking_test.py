...
x_train = train_df_lgbm[train_features]
y_train = train_df_lgbm.logerror
print(x_train.shape, y_train.shape)

test_df_lgbm['transactiondate'] = pd.Timestamp('2016-12-01')  # Dummy
test_df_lgbm = add_date_features(test_df)
x_test = test_df_lgbm[train_features]
...
L1_out = np.array([catb_pred,xgb_pred1,xgb_pred2,lgbm_pred])

L1_out_df = pd.DataFrame(L1_out)
L1_out_df.to_pickle('L1_out.pkl')
