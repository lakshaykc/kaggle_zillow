...
class house_model(object):
    ...
    def house_train(self,weights,light_gbm_params,xgb_params_1,num_boost_rounds_1,
        ...
        transc_df = train_org[["transactiondate"]].copy()
        transc_df["transactiondate"] = pd.to_datetime(transc_df["transactiondate"])
        transc_df["month"] = transc_df["transactiondate"].dt.month
        X_test = transc_df[transc_df["month"] >= 10].index
        X_train = transc_df[transc_df["month"] < 10].index

        train_set = train_org.iloc[X_train]
        test_set_org = train_org[["parcelid","transactiondate"]].iloc[X_test]
        ...
        if type == "test":
            test_set_ret = train_org[["parcelid","transactiondate","logerror"]].iloc[test_index]
...
if __name__ == '__main__':
    ...
    note = "Just XGB, on original dataset with 10 fold val"
    log_entry['Note'] = note
